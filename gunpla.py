	...
	def __calcular_danio(self,arma_elegida,armas_combinables=[]):
		...
		if(random.uniform(PROBABILIDAD_INICIO,PROBABILIDAD_FINAL)<=probabilidad):
			...
			if(armas_combinables):
				...
				#Gaston: Son iguales, lo unico que cambia es la probabilidad,
				#        es tan simple como tener un if melee: prob = PROBABILIDAD_MELEE else: prob = PROBABILIDAD_RANGO
				#        y el resto es siempre igual
				if(arma_combinable.get_tipo()==Arma.MELEE and random.uniform(PROBABILIDAD_INICIO,PROBABILIDAD_FINAL) <=PROBABILIDAD_MELEE):
					...
					return danio_reducido + self.__calcular_danio(arma_combinable,armas_combinables)
